...
class Grades {
    ...
    public function getGrades(int $id, string $subject) {
        $stmt = $this->conn->prepare("SELECT * FROM grade WHERE student_id = ? AND subject = ?");
        $stmt->execute([$id, $subject]);
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    }

    public function recordGrade(int $studentId, string $subject, string $quarter, float $grade) {
        ...
        if ($existing) {
            ...
            $sql = "INSERT INTO grade (student_id, subject, prelim, midterm, finals) VALUES (?, ?, ?, ?, ?)";
            $prelim  = $quarter === "Prelim"   ? $grade : null;
            $midterm = $quarter === "Midterms" ? $grade : null;
            $finals  = $quarter === "Finals"   ? $grade : null;

            $stmt = $this->conn->prepare($sql);
            return $stmt->execute([$studentId, $subject, $prelim, $midterm, $finals]);
        }
    }
}
